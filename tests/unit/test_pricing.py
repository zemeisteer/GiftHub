...
async def test_calculate_stars_price_and_rounding(db_session: AsyncSession):
    res = await pricing_service.get_authoritative_price(session=db_session, product_type="stars", amount=50)
...
async def test_bulk_discount_calculation(db_session: AsyncSession):
    ...
    res_small = await pricing_service.get_authoritative_price(session=db_session, product_type="stars", amount=50)
    res_large = await pricing_service.get_authoritative_price(session=db_session, product_type="stars", amount=1000)
...
async def test_create_and_validate_price_lock(db_session: AsyncSession):
    lock = await pricing_service.create_price_lock(
        session=db_session, product_type="stars", amount=100, user_id=12345678
    )
    ...
    validated = await pricing_service.validate_or_consume_price_lock(
        session=db_session, lock_id=lock.id, product_type="stars", amount=100
    )
...
async def test_expired_price_lock_raises_exception(db_session: AsyncSession):
    ...
    with pytest.raises(PriceExpiredError):
        await pricing_service.validate_or_consume_price_lock(
            session=db_session, lock_id="pl_expired_test_123", product_type="stars", amount=50
        )
