...
@pytest.mark.asyncio
async def test_click_prepare_concurrency_and_service_validation(db_session: AsyncSession, sample_user: User, monkeypatch):
    """
    Req 2 & 3:
...
async def test_payme_simultaneous_perform_and_cancel_concurrency(db_session: AsyncSession, sample_user: User):
    ...
    create_res = await payme_provider.handle_request(
        db_session,
        {"jsonrpc": "2.0", "id": 1, "method": "CreateTransaction", "params": create_params}
    )
    ...
    res_perf1 = await payme_provider.handle_request(
        db_session,
        {"jsonrpc": "2.0", "id": 2, "method": "PerformTransaction", "params": perform_params}
    )
    res_perf2 = await payme_provider.handle_request(
        db_session,
        {"jsonrpc": "2.0", "id": 3, "method": "PerformTransaction", "params": perform_params}
    )
    ...
    res_cancel1 = await payme_provider.handle_request(
        db_session,
        {"jsonrpc": "2.0", "id": 4, "method": "CancelTransaction", "params": cancel_params}
    )
    res_cancel2 = await payme_provider.handle_request(
        db_session,
        {"jsonrpc": "2.0", "id": 5, "method": "CancelTransaction", "params": cancel_params}
    )
...
async def test_outbox_multi_worker_claiming_and_backoff(db_session: AsyncSession):
    ...
    worker_a_events = await outbox_service.claim_pending_events(
        session=db_session,
        worker_id="worker_A",
        limit=1
    )
    ...
    worker_b_events = await outbox_service.claim_pending_events(
        session=db_session,
        worker_id="worker_B",
        limit=10
    )
    ...
    await outbox_service.mark_failed(
        session=db_session,
        event_id=claimed_by_a.id,
        error="Simulated network failure #1"
    )
    ...
    await outbox_service.mark_failed(
        session=db_session,
        event_id=claimed_by_a.id,
        error="Simulated network failure final"
    )
    await db_session.refresh(claimed_by_a)
    ...
    dlq_res = await db_session.execute(
        select(FailedJob).where(FailedJob.order_id == int(claimed_by_a.aggregate_id))
    )
...
async def test_refund_concurrency_and_database_uniqueness(db_session: AsyncSession, sample_user: User):
    ...
    res1 = await order_service.refund_order(
        session=db_session,
        order_id=order.id,
        reason="Fulfillment provider failure"
    )
    ...
    with pytest.raises(Exception):
        await order_service.refund_order(
            session=db_session,
            order_id=order.id,
            reason="Duplicate concurrent refund request"
        )
    ...
    tx_count = (await db_session.execute(
        select(func.count(WalletTransaction.id)).where(
            ...
            WalletTransaction.reference_id == order.order_code
        )
    )).scalar()
    assert tx_count == 1
...
async def test_demo_seed_data_strictly_gated(db_session: AsyncSession, monkeypatch):
    ...
    demo_codes = (await db_session.execute(
        select(PromoCode).where(PromoCode.code.in_(["START2026", "VIP10", "STARS50"]))
    )).scalars().all()
