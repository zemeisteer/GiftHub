...
def upgrade() -> None:
    ...
    if "orders" not in existing_tables:
        ...
        with op.batch_alter_table("orders") as batch_op:
            if "discount_amount" not in existing_cols:
                batch_op.add_column(
                    sa.Column("discount_amount", sa.Numeric(precision=18, scale=2), server_default="0.00")
                )
    ...
    if "referral_rewards" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "referred_user_id", sa.BigInteger(), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False
            ),
            sa.Column(
                "order_id", sa.Integer(), sa.ForeignKey("orders.id", ondelete="CASCADE"), unique=True, nullable=False
            ),
