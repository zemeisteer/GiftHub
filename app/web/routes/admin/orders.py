...
async def get_admin_orders(
    ...
    async with AsyncSessionLocal() as session:
        orders = await queries.list_all_orders(
            session, search=search, status=status, product_type=product_type, limit=limit, offset=offset
        )
        result = []
        for o in orders:
...
async def update_order_status_endpoint(
    order_id: int, req: OrderStatusUpdate, admin: User = Depends(require_permission(Permission.ORDERS_UPDATE))
):
    async with AsyncSessionLocal() as session:
        order = await queries.update_order_status(
            session=session, order_id=order_id, new_status=req.status, admin_id=admin.id, reason=req.reason
        )
        if not order:
            raise HTTPException(status_code=404, detail="Buyurtma topilmadi")

        refund_amount = (
            float(order.total_price) if req.status in ("cancel", OrderStatus.CANCELLED, OrderStatus.REFUNDED) else 0.0
        )
        asyncio.create_task(
            send_order_status_update_notification(
                bot=get_bot(),
...
async def refund_order_endpoint(
    order_id: int, req: RefundRequest, admin: User = Depends(require_permission(Permission.ORDERS_REFUND))
):
...
async def retry_fulfillment_endpoint(
    order_id: int, admin: User = Depends(require_permission(Permission.ORDERS_UPDATE))
):
...
async def mark_order_fulfilled_endpoint(
    ...
    async with AsyncSessionLocal() as session:
        order = await queries.update_order_fulfillment(
            session=session, order_id=order_id, fulfillment_status="fulfilled", status=OrderStatus.COMPLETED
        )
        if not order:
            raise HTTPException(status_code=404, detail="Buyurtma topilmadi")
        await queries.log_admin_action(
...
async def export_orders_csv(admin: User = Depends(require_permission(Permission.ORDERS_READ))):
    async with AsyncSessionLocal() as session:
        ...
        writer.writerow(
            ["ID", "Buyurtma kodi", "User ID", "Mahsulot", "Miqdor", "Summa (UZS)", "Tannarx (UZS)", "Holat", "Sana"]
        )
        for o in orders:
            writer.writerow(
                [
