...
async def get_admin_dashboard(admin: User = Depends(require_permission(Permission.ANALYTICS_READ))):
    async with AsyncSessionLocal() as session:
        ...
        new_users_week = (
            await session.execute(select(func.count(User.id)).where(User.created_at >= week_ago))
        ).scalar() or 0
        ...
        stars_sales = (
            await session.execute(
                select(func.sum(Order.total_price)).where(
                    Order.product_type == "stars", Order.status.in_([OrderStatus.COMPLETED, "done"])
                )
            )
        ).scalar() or 0.0
        stars_cost = (
            await session.execute(
                select(func.sum(Order.cost_price)).where(
                    Order.product_type == "stars", Order.status.in_([OrderStatus.COMPLETED, "done"])
                )
            )
        ).scalar() or 0.0
        ...
        prem_sales = (
            await session.execute(
                select(func.sum(Order.total_price)).where(
                    Order.product_type == "premium", Order.status.in_([OrderStatus.COMPLETED, "done"])
                )
            )
        ).scalar() or 0.0
        prem_cost = (
        ...
        gifts_sales = (
            await session.execute(
                select(func.sum(Order.total_price)).where(
                    Order.product_type == "gift", Order.status.in_([OrderStatus.COMPLETED, "done"])
                )
            )
        ).scalar() or 0.0
        gifts_cost = (
        ...
        orders_q = await session.execute(
            select(Order.total_price, Order.created_at, Order.completed_at).where(
                Order.status.in_([OrderStatus.COMPLETED, "done"]), Order.created_at >= month_ago
            )
        )
        ...
        for d in range(6, -1, -1):
            ...
            for row in done_orders:
                ...
                if dt and dt.date() == target_dt:
                    ...
                    day_count += 1
            weekly_chart.append(
                {"label": day_label, "date": target_dt.strftime("%d-%m"), "sales": day_sales, "count": day_count}
            )
        ...
        return {
            ...
            "breakdown": [
                {
                    "name": "Stars (jami)",
                    "sales": round(float(stars_sales)),
                    "cost": round(float(stars_cost)),
                    "profit": round(float(stars_sales) - float(stars_cost)),
                },
                {
                    "name": "Premium (jami)",
                    "sales": round(float(prem_sales)),
                    "cost": round(float(prem_cost)),
                    "profit": round(float(prem_sales) - float(prem_cost)),
                },
                {
                    "name": "Sovg'alar (jami)",
                    "sales": round(float(gifts_sales)),
                    "cost": round(float(gifts_cost)),
                    "profit": round(float(gifts_sales) - float(gifts_cost)),
                },
            ],
