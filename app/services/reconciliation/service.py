...
class ReconciliationService:
    ...
    async def run_reconciliation_audit(session: AsyncSession) -> ReconciliationReport:
        ...
        stmt_4 = (
            select(PaymentTransaction, Order)
            .join(Order, PaymentTransaction.order_id == Order.id)
            .where(and_(PaymentTransaction.status == "success", PaymentTransaction.amount != Order.total_price))
        )
        ...
        stmt_5 = (
            ...
            .where(and_(PaymentTransaction.status == "success", PaymentTransaction.order_id.is_not(None)))
        ...
        report = ReconciliationReport(
            ...
            status=status_str,
            summary_json={"checked_at": now.isoformat(), "discrepancies_found": len(discrepancies)},
            created_at=now,
        )
        session.add(report)
        await session.flush()
        ...
        await session.flush()
        logger.info(
            f"Reconciliation audit finished: {p_count} payments, {o_count} orders checked. {len(discrepancies)} discrepancies found."
        )
        return report
    ...
    async def get_dashboard_stats(session: AsyncSession) -> Dict[str, Any]:
        ...
        fulfilled_orders_count = (
            await session.scalar(select(func.count(Order.id)).where(Order.fulfillment_status == "fulfilled")) or 0
        )
        refunded_orders_count = (
            await session.scalar(select(func.count(Order.id)).where(Order.status == OrderStatus.REFUNDED.value)) or 0
        )

        # Unresolved discrepancies
        unresolved_disc_count = (
            await session.scalar(
